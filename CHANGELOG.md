...
### Added
- **`printf`** builtin -- `printf FORMAT [ARGUMENT]...` with `%s`, `%d`/`%i`, `%c` and `%%`, backslash escapes in FORMAT (`\n`, `\t`, `\r`, `\\`, `\a`, `\b`, `\f`, `\v`, `\"`, `\'`, and octal `\0ooo` / `\ooo`), and POSIX format reuse: the format is reapplied until the arguments run out, so `printf '%s\n' a b c` prints three lines, while a format with no conversions is printed once no matter how many arguments follow. Missing arguments consume empty (`%s`, `%c`) or zero (`%d`), and a non-numeric `%d` argument prints `printf: X: invalid number` on stderr, substitutes 0, and exits 1 -- bash's behavior, not an abort. Escapes are interpreted in FORMAT only, never in an argument: an argument is data. Deliberately unsupported, each a named error rather than silently wrong output: float conversions (`%f` and friends), field widths and precision (`%5d`, `%-10s`), `%b`, `%x`, and a trailing `%` with nothing after it: the subset agents actually type, with enough rope to diverge from bash elsewhere. `%d` accepts decimal, `0x` hex and leading-zero octal, each optionally signed; a leading zero commits the operand to octal, so `08` is an invalid number rather than eight, as in bash. A leading `--` ends the (nonexistent) options and is dropped, so `printf -- '%s\n' x` prints `x`. Output goes to `ctx.stdout`, so redirects and pipes compose (`printf 'line\n' > f` writes exactly `line\n`, `printf x | cat` works) and no trailing newline is ever added. One divergence follows from how termish already treats a failing command: `printf '%d\n' abc > f` aborts before the redirect is applied, so the substituted `0` lands in the transcript rather than in `f`, where bash would write it. The gap was expensive: agents write `printf '...\n' > f` as a matter of course, and scripts that passed on a real-bash rung died here with `printf: command not found`, so cross-rung conformance corpora had to avoid `printf` entirely (issue #18).
...
### Changed
- **Pipes and redirects carry bytes.** `cat bin.dat > copy.dat` on a 256-byte file holding every byte value used to write a 512-byte copy, silently: the payload between stages was `str`, so every byte that was not valid UTF-8 became the three-byte encoding of U+FFFD on its way through. It now writes 256 bytes. What a pipe hands the next stage, what `>` and `>>` write, what `< file` feeds in, and what a heredoc body becomes (encoded UTF-8) are all bytes. Decoding happens at boundaries and only once: a text builtin decodes its own stdin with `errors="replace"`, exactly as it already did for file content, and the transcript `execute()` returns is decoded from the accumulated bytes at the end -- it is a display artifact, so undecodable output shows U+FFFD there while the bytes that reached a file or a pipe are untouched. No builtin decodes and re-encodes content it is only moving: `cat`, `head`, `tail`, `tee`, `zcat`/`gzcat`, `gzip -c` and `gzip -dc` write bytes; `head`/`tail` split lines at `\n` at the byte level; `cp` and `mv` already worked on bytes. `wc -c` counts bytes (`printf 'é\n' | wc -c` is 3), and `wc -m` now counts characters (2) instead of being an alias for `-c`.
- **`zcat` / `gzip -d` read stdin** when given no file operands, so `cat file.gz | zcat` and `cat f.gz | gunzip -c` work and archive builtins compose in a pipeline; with no operands and nothing piped in, the usage error is unchanged. `gzip -c`'s compressed output is real gzip bytes rather than a latin-1 transliteration of them.
- **The handler contract is unchanged, and `.buffer` is the new binary path.** `ctx.stdin` and `ctx.stdout` are still text streams -- now `io.TextIOWrapper`s over the byte buffer, UTF-8 with `errors="replace"` (a text read of binary never raises), no newline translation (`\r\n` survives), and writes passed straight through -- so a handler that writes `ctx.stdout.write("...")` or reads `ctx.stdin.read()` needs no change. A handler that wants bytes uses `ctx.stdout.buffer.write(data)` / `ctx.stdin.buffer.read()`, which nothing could do before: an injected command producing binary had no way to emit it. Flush before switching between the two views of one stream. `CommandResult.stderr` stays a `str`: `2>file` writes it encoded UTF-8, `2>&1` merges it into the byte stream after whatever the handler wrote, and transcript diagnostics are unchanged. This is a 0.2.0-class change in behaviour -- a caller that was reading the corrupted-but-predictable output of a binary pipeline sees different bytes -- even though no handler has to be rewritten.
- **Control-flow keywords now raise `ParseError`** instead of being resolved as commands. `for f in a b; do echo $f; done` used to produce three unrelated failures -- a transcript of `for: command not found` and `do: command not found`, then a `TerminalError` for `done` with exit 127 -- none of which said that loops are unimplemented. The parser now stops at the first such word and raises one diagnostic naming it: `for: control flow is not supported; use xargs or find -exec for iteration` for `for` / `while` / `until` / `do` / `done`, `if: control flow is not supported; use && and || for conditionals` for `if` / `then` / `elif` / `else` / `fi`, and a bare `case: control flow is not supported` for `case` / `esac`. Function definitions are rejected the same way -- `function: ...` for the `function` keyword and `greet(): ...` for the `greet() { ... }` form -- each suggesting that reusable commands be injected through `execute()`'s `commands` argument. The rule is command position only, the same place the command name is resolved: `echo for`, `grep -r done .`, `ls if`, `echo hi > for` and a file literally named `for` all keep working, and a quoted keyword is an ordinary word (`'for'` resolves as a command name, as in bash). Heredoc bodies, extracted before tokenization, are unaffected. This is `ParseError` rather than `TerminalError` for the reason command substitution already is: the shell cannot run the script at all, so there is no exit code or partial transcript to report.
